...
fn rearrange_stacks_with_cratemover9000(input: &String) -> String {
...
fn rearrange_stacks_with_cratemover9001(input: &String) -> String {
    let (mut stacks, moves) = get_stacks_and_moves(input);
    perform_moves_with_cratemover9001(&mut stacks, moves);
    get_top_of_stacks(&stacks)
}

fn get_stacks_and_moves(input: &String) -> (Vec<Vec<char>>, Vec<Move>) {
    ...
    for line in input.lines() {
        if !line.is_empty() {
            let first_char = line.trim().chars().nth(0).unwrap();
            match first_char {
...
fn parse_stacks(count_of_stacks: usize, stack_lines: Vec<&str>) -> Vec<Vec<char>> {
    ...
    for stack_line in stack_lines {
        for i in 0..count_of_stacks {
            match stack_line.chars().nth(i * 4 + 1) {
                Some(c) => {
...
fn perform_moves_with_cratemover9000(stacks: &mut Vec<Vec<char>>, moves: Vec<Move>) {
    for m in moves {
        for _ in 0..m.count {
...
fn perform_moves_with_cratemover9001(stacks: &mut Vec<Vec<char>>, moves: Vec<Move>) {
    for m in moves {
